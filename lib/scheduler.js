...
export const Scheduler = {
    ...
    // Schedule an event to occur at a period of exactly n/d (1000 by default
    // to be used for fps, could be 60000 for bpm)
    ratio(f, n, d = 1000, t = 0) {
        this.events.add({ f, t, period: [d, n] });
    },
    ...
    schedule(from, to, inclusive) {
        ...
        for (const event of this.events.values()) {
            ...
            const d = Array.isArray(period) ? period[0] / period[1] : period;
            if (isNaN(d)) {
                // Single occurrence
                ...
                for (let i = Math.min(m, n); i <= Math.max(m, n); ++i) {
                    ...
                    if (((p > 0) || (p === 0 && inclusive === Inclusive)) && p <= 1) {
                        ...
                        if (Array.isArray(period)) {
                            ev.push(Math.round(period[1] * (ti - t) / period[0]));
                        }
                        schedule.push(ev);
                    }
                }

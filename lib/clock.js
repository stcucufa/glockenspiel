...
export const Clock = {
    ...
    // Create a new clock with a default rate, ticker and scheduler.
    create: create({
    ...
    get now() {
        return this.offset + (
            this.rate === 0 ?
                this.pauseTime :
                (this.tick.now - this.referenceTime) * this.rate
        );
    },

    // Start the clock just before zero.
    start() {
        this.referenceTime = this.tick.now;
    ...
    requestUpdate(force = false) {
        if (force || this.state === States.Running) {
            this.tick.requestUpdate(
                this,
                () => { this.update(); }
            );
        }
